...
from graphify.config import load_project_config, path_allowed, resolve_profile_name
...
def _resolve_scan_settings(
    ...
    *,
    ...
    active_profile = resolve_profile_name(config, profile)
    effective_follow = (
        config.follow_symlinks
        if config.follow_symlinks is not None
        else follow_symlinks
    )
    return config, active_profile, effective_follow
...
def detect(
    ...
    *,
    ...
    root = root.resolve()
    config, active_profile, effective_follow = _resolve_scan_settings(
        root,
    ...
    for scan_root in scan_paths:
        in_memory_tree = memory_dir.exists() and scan_root == memory_dir
        for dirpath, dirnames, filenames in os.walk(scan_root, followlinks=effective_follow):
            dp = Path(dirpath)
            if effective_follow and os.path.islink(dirpath):
            ...
            if not in_memory_tree:
                ...
                dirnames[:] = [
                    ...
                    and path_allowed(dp / d, root, config, profile=active_profile, is_dir=True)
                ]
    ...
    for p in all_files:
        ...
        if _is_ignored(p, root, ignore_patterns):
            continue
        if not in_memory and not path_allowed(p, root, config, profile=active_profile):
            continue
        if _is_sensitive(p):
...
def detect_incremental(
    ...
    *,
    ...
    """
    full = detect(root, follow_symlinks=follow_symlinks, profile=profile)
