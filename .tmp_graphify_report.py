...
def generate(
    ...
    if alignment:
        bits: list[str] = []
        if alignment.get("canonicalized_nodes"):
            bits.append(f"{alignment['canonicalized_nodes']} canonicalized")
        if alignment.get("bridge_edges"):
            bits.append(f"{alignment['bridge_edges']} bridge edges")
        if alignment.get("file_reference_nodes"):
            bits.append(f"{alignment['file_reference_nodes']} file references")
        if bits:
            lines.append(f"- Alignment: {' · '.join(bits)}")
    lines += [
        "",
    ...
    lines += ["", "## Communities"]
    from .analyze import _is_annotation_node as _ian
    from .analyze import _is_file_node as _ifn
    for cid, nodes in communities.items():
        label = community_labels.get(cid, f"Community {cid}")
        score = cohesion_scores.get(cid, 0.0)
        # Filter structural noise and low-signal annotation nodes from summaries.
        real_nodes = [n for n in nodes if not _ifn(G, n) and not _ian(G, n)]
    ...
    # --- Gaps section ---
    from .analyze import _is_annotation_node, _is_concept_node, _is_file_node
    ...
    isolated = [
        ...
        and not _is_annotation_node(G, n)
    ]
    thin_communities = {
        ...
        if thin_communities:
            for cid, nodes in thin_communities.items():
                ...
                node_labels = [
                    ...
                    for n in nodes
                    if not _is_annotation_node(G, n)
                ]
                if not node_labels:
                    continue
