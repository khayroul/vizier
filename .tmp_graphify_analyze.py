...
def _is_annotation_node(G: nx.Graph, node_id: str) -> bool:
    """Return True for low-signal narrative helper nodes extracted as evidence."""
    ...
    if len(label.split()) >= 8:
        return True
    if len(label) >= 90 and re.search(r"[,:.;]", label):
        return True
    return False
...
def god_nodes(G: nx.Graph, top_n: int = 10) -> list[dict]:
    ...
    for node_id, deg in sorted_nodes:
        if _is_file_node(G, node_id) or _is_concept_node(G, node_id) or _is_annotation_node(G, node_id):
            continue
        result.append({
            "id": node_id,
...
def _cross_file_surprises(G: nx.Graph, communities: dict[int, list[str]], top_n: int) -> list[dict]:
    ...
    for u, v, data in G.edges(data=True):
        ...
        if _is_file_node(G, u) or _is_file_node(G, v):
            continue
        if _is_annotation_node(G, u) or _is_annotation_node(G, v):
            continue
